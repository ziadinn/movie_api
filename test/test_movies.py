...
def test_get_movie_428():
    response = client.get("/movies/428")
    assert response.status_code == 200

    with open("test/movies/428.json", encoding="utf-8") as f:
        assert response.json() == json.load(f)
...
def test_sort_rating():
    response = client.get(
        "/movies/?name=an&sort=rating"
    )
    assert response.status_code == 200

    with open(
        "test/movies/name=an&sort=rating.json",
        encoding="utf-8",
    ) as f:
        assert response.json() == json.load(f)
